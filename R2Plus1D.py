...
class ChannelAttention(nn.Module):
    def __init__(self,in_channels,out_channels,kernel_size,padding,stride):
        ...
        self.avg_pool=nn.AdaptiveAvgPool3d((self.temporal,None,None))
        self.max_pool=nn.AdaptiveMaxPool3d((self.temporal,None,None))
        
        self.overloop_pool=nn.AvgPool3d(3,1,1) 
        self.fc1=nn.Conv3d(in_channels, intermed_channels, spatial_kernel_size,
                                      stride=spatial_stride, padding=spatial_padding)
        self.bn1 = nn.BatchNorm3d(intermed_channels)
        self.relu=nn.ReLU()
        self.fc2=nn.Conv3d(intermed_channels, out_channels, temporal_kernel_size,
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, downsample=False):
        ...
        if self.downsample:  
            ...
            self.ca=ChannelAttention(in_channels, out_channels, kernel_size, padding=padding, stride=2)
            self.downsampleconv = SpatioTemporalConv(in_channels, out_channels, 1, stride=2)
            self.downsamplebn = nn.BatchNorm3d(out_channels)
        else:
            #self.conv1 =  SpatioTemporalConv(in_channels, out_channels, kernel_size, padding=padding,stride=1)
            self.ca = ChannelAttention(in_channels, out_channels, kernel_size, padding=padding,stride=1)
