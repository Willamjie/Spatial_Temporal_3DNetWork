...
class SpatialAttention(nn.Module):
    def __init__(self,kernel_size,out_channels):
        ...
        self.conv1=nn.Conv3d(2+out_channels,out_channels,kernel_size,bias=False)
        self.sigmoid=nn.Sigmoid()
    def forward(self,x):
        avg_out=torch.mean(x,dim=1,keepdim=True)
        max_out,_=torch.max(x,dim=1,keepdim=True)
...
class FeatureLayer(nn.Module):
    ...
    def __init__(self, layer_sizes, input_channel=3):
        ...
        # self.conv3 = ResLayer(64, 128, 3, layer_sizes[1])     
        self.conv4 = ResLayer(64, 256, 3, layer_sizes[2], downsample=True)
