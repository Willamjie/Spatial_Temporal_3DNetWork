...
#         self.query_conv = nn.Conv3d(in_channels = in_channels, out_channels =out_channels , kernel_size= kernel_size,bias=False)
#         self.key_conv = nn.Conv3d(in_channels = in_channels, out_channels = out_channels , kernel_size= kernel_size)
#         self.value_conv = nn.Conv3d(in_channels = out_channels , out_channels = out_channels , kernel_size= kernel_size)
...
#         proj_query  = self.query_conv(x).view(self.out_channels,t,width*height).permute(0,2,1) # B X CX(N)
#         # print(proj_query.size())
#         proj_key =  self.key_conv(x).view(self.out_channels,t,width*height)# B X C x (*W*H)
#         # print(proj_key.size())
...
class ChannelAttention(nn.Module):
    def __init__(self,out_channels,kernel_size):
        ...
        self.avg_pool=nn.AdaptiveAvgPool3d((None,None,None))
        self.max_pool=nn.AdaptiveMaxPool3d((None,None,None))
...
class SpatioTemporalConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False, first_conv=False):
        ...
        # self.sa=Self_Attn(intermed_channels,out_channels,temporal_kernel_size)

        self.bn2 = nn.BatchNorm3d(out_channels)

        # self.temporal_atten=ChannelAttention(out_channels,1)
...
if __name__ == "__main__":
    inputs = torch.rand(1, 3, 32, 112, 112).to(0)
    net = R2Plus1D(101, (3, 4, 6, 3)).to(0)
