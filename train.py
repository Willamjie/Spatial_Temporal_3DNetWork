...
def processor(sample):
    ...
    classes = model(data)
    loss = loss_criterion(classes, labels.type(torch.cuda.LongTensor))
    return loss, classes
...
def on_end_epoch(state):
    ...
    data_frame.to_csv('statistics/{}_{}_{}_results.csv'.format(DATA_TYPE, MODEL_TYPE,results['train_top5_accuracy']), index_label='epoch')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Activity Recognition Model')
    parser.add_argument('--data_type', default='hmdb51', type=str, choices=['ucf101', 'hmdb51', 'kinetics600'],
                        help='dataset type')
    ...
    parser.add_argument('--num_epochs', default=50, type=int, help='training epoch number')
    ...
    optimizer = optim.Adam(optim_configs, lr=1e-4, weight_decay=5e-3)
